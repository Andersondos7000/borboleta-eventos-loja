...
class VisualPurchaseTestLocal {
  ...
  async initBrowser() {
    try {
      ...
      this.log('info', '👀 VOCÊ VERÁ O NAVEGADOR ABRINDO EM ALGUNS SEGUNDOS!');
      ...
      this.log('info', '🎬 AGORA VOCÊ PODE VER O NAVEGADOR EM AÇÃO!');
  ...
  async testAppAccess() {
    ...
    this.log('info', `\n🧪 Executando: ${testName}`);
    this.log('info', '🌐 Acessando a aplicação...');
    ...
    try {
      ...
      this.log('info', '⏳ Aguardando 3 segundos para você ver a página...');
  ...
  async testCheckoutNavigation() {
    ...
    this.log('info', `\n🧪 Executando: ${testName}`);
    this.log('info', '🛒 Navegando para a página de checkout...');
    ...
    try {
      ...
      // Aguardar um pouco para visualização
      this.log('info', '⏳ Aguardando 3 segundos para você ver a página de checkout...');
      await this.page.waitForTimeout(3000);
  ...
  async testFormElements() {
    ...
    this.log('info', `\n🧪 Executando: ${testName}`);
    this.log('info', '📝 Procurando elementos do formulário...');
    
    try {
      // Aguardar um pouco para a página carregar completamente
      this.log('info', '⏳ Aguardando página carregar completamente...');
      await this.page.waitForTimeout(3000);
      ...
      for (const [field, selectorList] of Object.entries(selectors)) {
        ...
        for (const selector of selectorList) {
          try {
            ...
            // Destacar elementos encontrados visualmente
      ...
      // Aguardar para você ver os elementos destacados
      this.log('info', '👀 Elementos encontrados foram destacados em vermelho/amarelo!');
      this.log('info', '⏳ Aguardando 5 segundos para você ver os elementos...');
  ...
  async testFormFilling() {
    ...
    this.log('info', `\n🧪 Executando: ${testName}`);
    this.log('info', '✏️  Preenchendo formulário...');
    ...
    try {
      ...
      this.log('info', '📝 Tentando preencher campo NOME...');
      try {
        ...
        for (const selector of nomeSelectors) {
          ...
          if (element) {
            ...
            await this.page.waitForTimeout(2000); // Pausa para visualização
      ...
      this.log('info', '📧 Tentando preencher campo EMAIL...');
      try {
      ...
      this.log('info', '📞 Tentando preencher campo TELEFONE...');
      try {
  ...
  async testFormSubmission() {
    ...
    this.log('info', `\n🧪 Executando: ${testName}`);
    this.log('info', '🚀 Tentando submeter formulário...');
      ...
      for (const selector of submitSelectors) {
        try {
          ...
          if (element) {
            ...
            this.log('info', '🎯 Botão de submit encontrado e destacado!');
  ...
  async generateReport() {
    ...
    const reportFile = path.join(reportsDir, `visual-purchase-test-local-${Date.now()}.json`);
    ...
    // Resumo final
    this.log('info', '\n📊 RESUMO DO TESTE VISUAL LOCAL:');
    this.log('info', `⏱️  Duração: ${Math.round(this.results.duration / 1000)}s`);
  ...
  async cleanup() {
    try {
      if (this.browser) {
        this.log('info', '⏳ Aguardando 10 segundos antes de fechar o navegador...');
        this.log('info', '👀 Você pode ver os resultados finais!');
  ...
  async run() {
    try {
      this.log('info', '🚀 Iniciando teste VISUAL LOCAL de compras...');
      this.log('info', '👀 ATENÇÃO: O navegador será aberto em modo VISUAL no seu Windows!');
      this.log('info', '🐌 Ações serão executadas com atraso para visualização');
      this.log('info', '🎬 Prepare-se para ver o navegador em ação!');
      ...
      this.log('info', '⏳ Aguardando 3 segundos para você ver o navegador...');
      await this.page.waitForTimeout(3000);
...
if (require.main === module) {
  const test = new VisualPurchaseTestLocal();
  test.run().then(() => {
    process.exit(0);
  }).catch((error) => {
...
module.exports = VisualPurchaseTestLocal;